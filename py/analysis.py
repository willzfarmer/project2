...
def main():
    ...
    jobs.append(
            jobServer.submit(compression,
                            ...
                            (create_grayscale, dst, create_S),
                            ('numpy', 'scipy.misc'))
            ) # Add a new job to compress our third image

    # Analyze Compression Effectiveness
    print('Generating Compression Effectiveness')
    comp_effect(image1, image2, image3)
...
def visualize_s():
    ...
    det_axes.plot(nrange, yrange, 'ro') # Add points
...
def comp_effect(image1, image2, image3):
    '''
    ...
    try:
        ...
        g3 = create_grayscale(image3.copy()) # Create grayscale from copy of 2

        domain1 = numpy.arange(0.0, 1.01, 0.01) # Range of p values
        domain2 = numpy.arange(0.0, 1.01, 0.01) # Range of p values
        domain3 = numpy.arange(0.0, 1.01, 0.01) # Range of p values

        # Parallelize System and generate range
        local_jobs = []
        local_jobs.append(jobServer.submit(get_yrange,
                        (domain1, g1),
        ...
        for job in local_jobs:
            results.append(job())
        count_y1 = results[0][0] # Assign variables
        rat_y1   = results[0][1]
        count_y2 = results[1][0]
        rat_y2   = results[1][1]
        count_y3 = results[2][0]
        rat_y3   = results[2][1]
        ...
        count_axes.plot(domain3, count_y3, label='Image 3')
        ...
        ratio_axes.plot(domain3, rat_y3, label='Image 3')
